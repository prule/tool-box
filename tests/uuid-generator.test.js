...
const source = readFileSync(resolve(__dirname, '../toolbox/tools/uuid-generator.logic.js'), 'utf8');
...
describe('generate', () => {
    ...
    describe('v3 / v5 validation', () => {
        ...
        it.each(['v3', 'v5'])('errors on malformed namespace (%s)', (version) => {
            const r = generate({ version, namespace: 'not-a-uuid', name: 'foo' }, makeMockLib());
        ...
        it.each(['v3', 'v5'])('errors on empty-string name (%s)', (version) => {
            const r = generate({ version, namespace: DNS_NAMESPACE, name: '' }, makeMockLib());
            expect(r).toEqual({ ok: false, error: 'Please provide a Name.' });
        });
    });
    ...
    describe('output format', () => {
        ...
        it('mock outputs satisfy isValidUuid', () => {
            ...
            for (const v of ['v3', 'v5']) {
                const r = generate({ version: v, namespace: DNS_NAMESPACE, name: 'x' }, lib);
                expect(r.ok).toBe(true);
                expect(isValidUuid(r.uuid)).toBe(true);
            }
        });
